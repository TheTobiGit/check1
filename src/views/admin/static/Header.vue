<template>
  <section class="flex justify-between h-[11vh] items-center px-10">
    <div class="h-full">
      <router-link :to="'/admin'" class="h-full flex items-center"
        ><img src="../../../assets/logo.png" alt="" class="h-[80%]"
      /></router-link>
    </div>
    <button
      class="flex gap-3 items-center cursor-pointer uppercase"
      @click="signout"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        height="40"
        viewBox="0 -960 960 960"
        width="48"
      >
        <path
          d="M180-120q-24 0-42-18t-18-42v-600q0-24 18-42t42-18h299v60H180v600h299v60H180Zm486-185-43-43 102-102H360v-60h363L621-612l43-43 176 176-174 174Z"
        />
      </svg>
      {{ status }}
    </button>
  </section>
</template>

<script>
import router from "../../../routes";
import { logout } from "../../../stores/UserData";

export default {
  setup() {
    const status = "logout";
    const signout = () => logout().then(() => router.push("/login"));

    return {
      status,
      signout,
    };
  },
};
</script>
